...
const initialTitleChars = [
    ...initialTitle.querySelectorAll('span.char')
];
...
// it's position
let centerTitlePosition = {
    x: centerTitle.offsetLeft,
    y: centerTitle.offsetTop
};
const centerTitleChars = [
    ...centerTitle.querySelectorAll('span.char')
];
...
const repeatedCenterTitleChars = [
    ...repeatedCenterTitle.querySelectorAll('span.char')
];
// top title element and it's letters
const topTitle = document.querySelector('#top-title');
const topTitleChars = [
    ...topTitle.querySelectorAll('span.char')
];
// it's position
let topTitlePosition = {
    x: topTitle.offsetLeft,
    y: topTitle.offsetTop
};
// bottom title element  and it's letters
const bottomTitle = document.querySelector('#bottom-title');
const bottomTitleChars = [
    ...bottomTitle.querySelectorAll('span.char')
];
// it's position
let bottomTitlePosition = {
    x: bottomTitle.offsetLeft,
    y: bottomTitle.offsetTop
};
// rows of titles
const rows = {
...
Object.keys(rows).forEach((row, pos)=>{
    rowsTitles[row] = [
        ...rows[row].querySelectorAll('.intro__title')
    ];
});
...
Object.keys(rows).forEach((row, pos)=>{
    contentSectionsPhotos[row] = [
        ...contentSections[pos].querySelectorAll('.strip__img')
    ];
...
const getRowsPosition = ()=>{
    let positions = {};
    for(const row in rowsTitles)if (rowsTitles.hasOwnProperty(row)) positions[row] = {
        x: rowsTitles[row][0].offsetLeft,
        y: rowsTitles[row][0].offsetTop
    };
    return positions;
};
...
const introTimeline = (0, _gsap.gsap).timeline({
    ...
    // add initial delay
    delay: 0.4
}).addLabel('start', 0)// step1: switch between the large "center title" and the smaller "center title" element
.to(initialTitleChars, {
    y: (pos)=>pos % 2 ? '100%' : '-100%',
    stagger: 0.02
}, 'start').to(centerTitleChars, {
    startAt: {
        y: (pos)=>pos % 2 ? '100%' : '-100%',
        opacity: 1
    },
...
}, 'start+=0.15').addLabel('step2', '>')// even "center title" chars will animate to the top
.to(centerTitleChars.filter((_, i)=>i % 2 == 0), {
    y: topTitlePosition.y - centerTitlePosition.y,
    stagger: 0.08
}, 'step2')// odd "top title" chars slide in
.to(topTitleChars.filter((_, i)=>i % 2 != 0), {
    ...
    stagger: 0.08,
    onComplete: ()=>{
        (0, _gsap.gsap).set(centerTitleChars.filter((_, i)=>i % 2 == 0), {
            opacity: 0
        });
        (0, _gsap.gsap).set(topTitleChars.filter((_, i)=>i % 2 == 0), {
            opacity: 1
        });
    }
}, 'step2')// odd "center title" chars will animate to the bottom
.to(centerTitleChars.filter((_, i)=>i % 2 != 0), {
    y: bottomTitlePosition.y - centerTitlePosition.y,
    stagger: 0.08
}, 'step2')// even "bottom title" chars slide in
.to(bottomTitleChars.filter((_, i)=>i % 2 == 0), {
    ...
    onComplete: ()=>{
        ...
        (0, _gsap.gsap).set(bottomTitleChars.filter((_, i)=>i % 2 != 0), {
            opacity: 1
        });
    }
}, 'step2')// "repeated center title" chars slide in to fill the gaps 
.to(repeatedCenterTitleChars, {
    startAt: {
        y: (pos)=>pos % 2 == 0 ? '100%' : '-100%',
        opacity: 1
    },
    y: '0%',
    stagger: (index)=>index % 2 == 0 ? 0.08 * (index / 2) : 0.08 * (index - (index - 1) / 2 - 1)
}, 'step2').addLabel('step2End', '>');
...
elemsArr.forEach((el, pos)=>{
    introTimeline.addLabel('step3', 'step2End+=' + pos * delay).to(el, {
        duration: 0.5,
    ...
    if (pos) introTimeline.set(pos === 1 ? rowsTitles.center : rowsTitles.bottom, {
        ease: 'expo',
        opacity: 0,
        stagger: 0.09
    }, 'step3+=1').to(pos === 1 ? rowsTitles.midTop : rowsTitles.midBottom, {
        ease: 'expo',
        startAt: {
        ...
        y: 0,
        stagger: 0.09
    }, 'step3+=1').to(pos === 1 ? rowsTitles.center : rowsTitles.bottom, {
        ease: 'expo',
        startAt: {
            y: '100%',
            opacity: 1
...
Object.keys(rows).forEach((row, pos)=>{
    contentSections[pos].addEventListener('mouseenter', ()=>{
        ...
        if (hoverOutTimeline[pos]) hoverOutTimeline[pos].kill();
        hoverInTimeline[pos] = (0, _gsap.gsap).timeline({
            defaults: {
                duration: 0.9,
                ease: 'expo'
            }
        }).addLabel('start', 0).set(rows[row], {
            opacity: 0
        }, 'start').set(contentSectionsHeadings[row], {
            pointerEvents: 'auto'
        }, 'start').to(contentSectionsHeadings[row], {
            opacity: 1
        }, 'start').to(contentSectionsHeadings[row].children, {
            startAt: {
                opacity: 0,
                y: '-100%'
            },
            ...
            opacity: 1,
            stagger: -0.06
        }, 'start').to(contentSectionsPhotos[row], {
            scale: 1.15
        }, 'start');
    });
    contentSections[pos].addEventListener('mouseleave', ()=>{
        if (introTimeline.progress() < 0.8) return;
        if (hoverInTimeline[pos]) hoverInTimeline[pos].kill();
        hoverOutTimeline[pos] = (0, _gsap.gsap).timeline({
            defaults: {
                duration: 0.9,
                ease: 'expo'
            }
        }).addLabel('start', 0).to(rows[row], {
            opacity: 1
        }, 'start').set(contentSectionsHeadings[row], {
            pointerEvents: 'none'
        }, 'start').set(contentSectionsHeadings[row].children, {
            opacity: 0
        }, 'start').to(contentSectionsPhotos[row], {
            scale: 1
        }, 'start');
...
(0, _utils.preloadImages)('.strip__img').then(()=>{
    document.body.classList.remove('loading');
    introTimeline.play();
});

},{"./utils":"bMpAD","gsap":"9F7Z6","splitting/dist/splitting.css":"3uR7n","splitting/dist/splitting-cells.css":"7jeGL","splitting":"6m3bh","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"bMpAD":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
},{}]},["Hyt4T","6kb64"], "6kb64", "parcelRequire96d7")

//# sourceMappingURL=RepetitiveTypography.6528c13b.js.map
